...
Nimbus is organized into thirteen modules:
...
nimbus-cli (Remote CLI)
├── NimbusCli           → Entry point, arg parsing, native access relaunch
├── CliConfig           → Connection profiles (~/.nimbus/cli.json)
├── CliConsole          → JLine3 REPL, event display, screen sessions
├── CompletionClient    → REST client for POST /api/console/complete
├── StreamClient        → WebSocket client for /api/console/stream
└── OutputRenderer      → Typed output lines → ANSI, event formatting
