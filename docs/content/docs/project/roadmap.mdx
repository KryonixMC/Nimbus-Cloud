...
| Goal | Why it matters |
|------|----------------|
| **Fill in operator workflows** | Things every network needs but nobody should have to glue together from third-party plugins — reports/appeals, chat filtering, staff tooling, analytics. Each lands as its own module so operators can opt in. Punishments, resource packs, and dashboard auth have already shipped |
...
| **Multi-user dashboard maturity** | v0.11 shipped the auth core — next is operational depth: audit-log view of auth events, admin-facing user management, per-user API tokens, configurable session TTL per group, and webhook delivery for magic links (Discord / email) alongside the in-game path |
...
- **Cluster-aware backups** — stream tar+zstd archives from agent nodes to the controller. Phase 1 of the backup module (0.9.1) ships local-only; agent-node services are currently skipped with `PARTIAL` status. Design notes are parked in the GitHub issue tracker and will be revived when real multi-node users surface the need
- **Agent-node Docker support** — the Docker module (0.10.0) ships controller-only; agents still run bare processes. Needs a small cluster protocol tweak so agents can report container runtime info in heartbeats. Tracked for a follow-up phase alongside pre-built image support and Windows named-pipe sockets
