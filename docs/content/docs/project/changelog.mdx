...
- **Strict-TOML-Warnungen (`[controller] strict_config`)** — Controller- und Modul-Konfigurationen (`nimbus.toml`, `groups/*.toml`, `dedicated/*.toml`, `config/modules/syncproxy/{motd,tablist,chat}.toml`, `config/modules/auth/auth.toml`, `config/modules/docker/docker.toml`, `agent.toml`) loggen beim Start jetzt für jeden unbekannten Key eine eigene WARN-Zeile mit Key-Name, Scope und dem Hinweis *"will be an error in Nimbus 1.0"*. Schützt vor stummen Vertippern (`tokne = "…"` statt `token`) und forward-kompatiblen Abwärtsänderungen, die bisher ohne Rückmeldung ignoriert wurden. Opt-in-Fail-Mode über `[controller] strict_config = true` — dann wird ein unbekannter Key zum `ConfigException`-Startup-Abbruch statt nur einer Warnung. Default in 0.13 ist `false` (warn-only), in Nimbus 1.0 wird der Default auf `true` gedreht. User-editierbare Message-Template-Dateien (`AuthMessages`, `PunishmentsMessages`) bleiben bewusst lenient, damit neue Platzhalter-Keys aus künftigen Versionen nicht rückwirkend Warnungen auf bestehenden Dateien auslösen
- **Cluster Protocol-Version Handshake** — `ClusterMessage.AuthRequest` und `AuthResponse` tragen jetzt ein explizites `protocolVersion`-Feld (aktuell `CURRENT_PROTOCOL_VERSION = 1`). Beim Handshake vergleicht der Controller die vom Agent gemeldete Version mit der eigenen *bevor* der Token geprüft wird. Mismatch führt zu `AuthResponse(accepted = false, reason = "protocol version mismatch: agent=X, controller=Y", protocolVersion = …)` und einem WS-Close mit `VIOLATED_POLICY`. Auf Agent-Seite erkennt `AgentRuntime` den Fatal-Reject über einen lokalen Versionsvergleich, wirft eine interne `ProtocolVersionMismatchException` und **bricht die Reconnect-Schleife sofort ab** (`running = false`, kein exponentielles Backoff). Verhindert das bisherige Reconnect-Churn bei inkompatiblen Deployments — statt stundenlang stumm im Kreis zu laufen, loggt der Agent einmal einen klaren ERROR mit der Aufforderung, beide Nimbus-Installationen anzugleichen, und beendet sich. Die Feld-Defaults (`= 1`) plus `ignoreUnknownKeys = true` im `clusterJson` halten den Rolling-Upgrade-Pfad zwischen 0.12.x- und 0.13.0-Knoten intakt. Bump-Regel (auch im Companion-Comment von `ClusterMessage`): Version nur anheben, wenn ein bestehendes Feld semantisch geändert, entfernt oder umbenannt wird — neue `@SerialName`-Message-Typen brauchen keinen Bump

### Changed

- **Module-API Paket umbenannt** — alle öffentlichen Modul-API-Typen (`NimbusModule`, `ModuleContext`, `ModuleCommand`, `AuthPrincipal`, `PermissionSet`, `PluginDeployment`, `PluginTarget`, `Migration`, `DoctorCheck`, `DoctorLevel`, `DoctorFinding`, `CommandCaller`, `CommandOutput`, `SubcommandMeta`, `CompletionMeta`, `CompletionType`, `AuthLevel`, `DashboardConfig`, `DashboardSection`, `SessionValidator` sowie die Extensions `hasPermission` und `service<T>()`) sind vom Paket `dev.nimbuspowered.nimbus.module` nach `dev.nimbuspowered.nimbus.module.api` verschoben worden. Ziel der Umstellung: klarere Trennung zwischen öffentlicher Modul-API und core-internen Typen (`ModuleManager`, `ModuleContextImpl`, `ModuleInfo`).

### Migration

- **Drittanbieter-Module:** Die bisherigen Imports funktionieren dank Deprecated-Typealiases weiterhin, lösen aber Compiler-Warnungen aus. Bitte alle Imports der Form `dev.nimbuspowered.nimbus.module.<Typ>` auf `dev.nimbuspowered.nimbus.module.api.<Typ>` umstellen. Die Kompatibilitäts-Shim wird in Nimbus 1.0 entfernt.
