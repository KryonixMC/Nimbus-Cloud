...
| `reconciliation_delay` | Int | `10000` | Milliseconds to wait for agent reconnections before starting services. During this window, agents report recovered services. Only after the delay does `startMinimumInstances()` run, preventing duplicate service starts. |
