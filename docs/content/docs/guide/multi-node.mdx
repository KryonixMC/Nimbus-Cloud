...
<Callout title="TLS Encryption" type="info">
Cluster TLS is **enabled by default** (`tls_enabled = true`). If no keystore is configured, Nimbus auto-generates a self-signed certificate on first startup. Agents connect via `wss://` and should set `tls_verify = false` when using self-signed certificates. For custom certificates, set `keystore_path` and `keystore_password`. See [nimbus.toml Reference — cluster](/docs/config/nimbus-toml#cluster) for details.
</Callout>
...
<Callout title="Manual setup" type="info">
