...
- **Paper/Pufferfish/Purpur/Leaf** — Enables velocity support in `paper-global.yml` and copies the forwarding secret
...
```text title="Nimbus — Auto-Patching"
...
[18:00:04] ✓ Velocity updated: 3.4.0 → 3.5.0 (restart proxy to apply)
```
...
```java title="SDK — Tab Name Override"
...
✓ Added jonas to maintenance whitelist.
```
...
- **Modded client routing**: NeoForge/Forge/Fabric clients are automatically detected via `player.getModInfo()` and routed to a server from a modded group instead of the Paper lobby. This prevents the "incompatible mods" kick that happens when NeoForge clients connect to vanilla/Paper servers
...
1. **Velocity config patching** — `announce-forge = true` is set in `velocity.toml` so Velocity advertises Forge compatibility in the server list ping. Connection and read timeouts are increased to handle large mod lists
2. **Modded client routing** — The bridge plugin detects modded clients on login (via Velocity's `ModInfo` API) and routes them to the least-loaded server from a modded group, instead of the Paper lobby
3. **Forwarding mods** — [proxy-compatible-forge](https://modrinth.com/mod/proxy-compatible-forge) or [FabricProxy-Lite](https://modrinth.com/mod/fabricproxy-lite) are auto-installed on modded backends (see [Automatic Proxy Forwarding Mods](#automatic-proxy-forwarding-mods))
...
```toml title="config/groups/modded-lobby.toml"
[group]
name = "ModdedLobby"
software = "NEOFORGE"    # NeoForge players land here automatically
version = "1.21.1"
modloader_version = "21.1.172"
```

Vanilla clients → `Lobby-1`, NeoForge clients → `ModdedLobby-1`. Fully automatic, no configuration needed beyond the group definitions.
...
2. **Downloads Floodgate** — Deployed to both proxy and all Paper/Purpur/Leaf/Folia backends
