...
The Punishments module adds the `punish` console command (and `/cloud punish …` in-game via the Bridge) for managing network-wide bans, mutes, kicks, and warnings. See the [Punishments guide](/docs/guide/punishments) for the full concept overview.
...
See the [Resource Packs guide](/docs/guide/resourcepacks) for the backend plugin's behaviour, the public download endpoint, and telemetry.
...
See [Dedicated Services](/docs/guide/dedicated-services) for full setup documentation.
...
See the [Doctor Guide](/docs/guide/doctor) for the full check catalogue and how to extend it.
...
<Callout type="info">
The scaling engine pauses while a stress test is active — simulated players won't trigger real scale-ups. Proxy groups are never simulated; only backends. See [Stress Testing](/docs/guide/stress-testing).
</Callout>
...
These commands are registered by controller modules and are only available when the corresponding module is installed. See [`modules`](#modules) to install.
...
Full workflows, cron syntax, and the GFS retention model live in the [Backup Guide](/docs/guide/backup).
...
See the [Scaling Guide](/docs/guide/scaling).
...
See [Cluster TLS & Security](/docs/guide/cluster-tls) for the trust-on-first-use flow and cert rotation instructions.
