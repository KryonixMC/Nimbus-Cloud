...
import dev.kryonix.nimbus.protocol.clusterJson
            ...
            try {
                ...
                if (nodeId != null) {
                    ...
                    if (node != null) {
                        ...
                        eventBus.emit(dev.kryonix.nimbus.event.NimbusEvent.NodeDisconnected(nodeId))
                    }
                }
            }
        }
                ...
                if (service != null) {
                    ...
                    when (newState) {
                        ServiceState.STOPPED -> eventBus.emit(
                            dev.kryonix.nimbus.event.NimbusEvent.ServiceStopped(message.serviceName))
                        ServiceState.CRASHED -> eventBus.emit(
                            dev.kryonix.nimbus.event.NimbusEvent.ServiceCrashed(message.serviceName, -1, 0))
                        else -> {}
                    }
                }
            }
